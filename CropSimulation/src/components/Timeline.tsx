    ...
    <div className="bg-white rounded-xl shadow-lg p-6">
      ...
      <div className="mt-6 pt-4 border-t border-gray-200">
        <div className="flex justify-between text-sm text-gray-600 mb-2">
          <span>Progress</span>
          <span>{previousStages.length + (currentStage ? 1 : 0)} / {previousStages.length + (currentStage ? 1 : 0) + upcomingStages.length} stages</span>
        </div>
        <div className="w-full bg-gray-200 rounded-full h-2">
          <div 
            className="bg-gradient-to-r from-green-400 to-green-600 h-2 rounded-full transition-all duration-500"
            style={{ 
              width: `${((previousStages.length + (currentStage ? 1 : 0)) / (previousStages.length + (currentStage ? 1 : 0) + upcomingStages.length)) * 100}%` 
            }}
          ></div>
        </div>
      </div>
