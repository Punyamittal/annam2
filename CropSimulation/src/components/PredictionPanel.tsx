...
const PredictionPanel: React.FC<PredictionPanelProps> = ({ 
  ...
  // Calculate potential yield with current parameters (simulate to maturity)
  let potentialYield = 0;
  if (cropName && currentDay >= 0) {
    try {
      const model = new PCSeModel(cropName);
      // Simulate from current day to maturity with current parameters
      let day = currentDay;
      let mature = false;
      while (!mature && day < currentDay + 200) {
      ...
      potentialYield = model.getYieldPrediction();
    } catch {}
  }

  // Calculate optimal yield (simulate to maturity with optimal parameters)
  let optimalYield = 0;
  if (cropName) {
    try {
      const model = new PCSeModel(cropName);
      optimalYield = model.getOptimalYieldPrediction();
    } catch {}
  }
  ...
  if (showOptimum) {
    try {
      ...
      maxDays = Math.max(...Object.values(timings));
    } catch {}
  }
      ...
      {showOptimum ? (
        <div className="mb-6 p-4 bg-gradient-to-r from-green-50 to-blue-50 rounded-lg border border-green-200">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              <p className="text-2xl font-bold text-blue-700">{optimalYield.toFixed(1)}</p>
        ...
        <>
          ...
          {readyForHarvest ? (
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <p className="text-2xl font-bold text-blue-700">
                    {harvestPrediction.finalYield.toFixed(1)}
                  </p>
                  <p className="text-sm text-blue-600">Final Yield (t/ha)</p>
            ...
            <div className="mb-6 p-4 bg-gradient-to-r from-green-50 to-blue-50 rounded-lg border border-green-200">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <p className="text-2xl font-bold text-green-700">{harvestPrediction.daysRemaining}</p>
                ...
                <div>
                  <div className="flex flex-col">
                    <span className="text-xs text-blue-500 font-semibold mb-1">Potential Yield (Current Parameters)</span>
                    <span className="text-2xl font-bold text-blue-700">{potentialYield.toFixed(1)}</span>
                    <span className="text-xs text-blue-500 mb-2">t/ha</span>
                    <span className="text-xs text-green-500 font-semibold mb-1 mt-2">Optimal Yield (Best Conditions)</span>
                    <span className="text-2xl font-bold text-green-700">{optimalYield.toFixed(1)}</span>
